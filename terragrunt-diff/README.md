...
> ## Terragrunt Plan
>
> **2 module(s) with changes** — 1 to add, 1 to change, 1 to destroy
>
...
- name: Parse and summarize plan
  uses: your-org/terragrunt-diff@v1
  with:
...
| `changes_json` | JSON array of changed modules: `[{ "module": "...", "toAdd": 0, "toChange": 1, "toDestroy": 0 }]` |
...
When `mode` is `comment` or `both`, the action posts a comment on the open PR. On re-runs it **updates the same comment** instead of creating a new one — no spam on every push.
...
The action understands the structured log format emitted by `terragrunt run --all`:
...
Only `STDOUT` lines are processed. `INFO`, `STDERR`, and `WARN` lines (init output, provider downloads, etc.) are discarded.
